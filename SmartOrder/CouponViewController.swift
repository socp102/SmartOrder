...
class CouponViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let data = ["data1": 123] as [String : Any]
        firebaseCommunicator.addData(collectionName: "coupon", documentName: "002", data: data) { (result, error) in
            if let error = error {
                print("error: \(error)")
            } else {
                print("result: \(result!)")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 4.0) {
            self.firebaseCommunicator.loadData(collectionName: "coupon", documentName: "002") { (results, error) in
                if let error = error {
                    ...
                    print("load 002 results: \(results!)")
                }
            }
        }
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 4.0) {
            self.firebaseCommunicator.loadData(collectionName: "coupon", field: "data1", condition: "123", completion: { (results, error) in
                if let error = error {
                    print("error: \(error)")
                } else {
